...
export default function StatsSection() {
  ...
  useEffect(() => {
    ...
    fetch("https://leetcode.com/u/animesh251001/")
      .then(res => res.text())
      .then(html => {
        const doc = new DOMParser().parseFromString(html, "text/html");
        const totalSolved =
          Number(doc.querySelector('span.text-\\[30px\\]')?.textContent) || 632;
    ...
    const leetRef = leetIntervalRef.current;
    const blogRef = blogIntervalRef.current;

    return () => {
      if (leetRef) clearInterval(leetRef);
      if (blogRef) clearInterval(blogRef);
    };
  }, [hasAnimated]);
  const stats = [
    {
      ...
      value: leetcodeCount,
      link: "https://leetcode.com/u/animesh251001/",
